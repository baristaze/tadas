...
class TaskStorageContract:
    ...
    async def test_round_trip_and_update_by_copy(self, storage: TasksStorageInterface) -> None:
        ...
        assert await storage.read_open_tasks(org, None, limit=10) == []
        assert await storage.read_done_tasks(org, None, None, limit=10) == [done]
    ...
    async def test_reads_are_tenant_scoped(self, storage: TasksStorageInterface) -> None:
        ...
        assert await storage.read_open_tasks(org_b, None, limit=10) == []
    ...
    async def test_open_list_sorts_by_position_hides_deleted_and_clamps(
        ...
        for task in tasks:
            await storage.write_task(org, task)
        listed = await storage.read_open_tasks(org, None, limit=10)
        assert [t.title for t in listed] == ["t1", "t2", "t0"]
        assert len(await storage.read_open_tasks(org, None, limit=2)) == 2
        ...
        assert [t.title for t in await storage.read_open_tasks(org, None, limit=10)] == ["t2", "t0"]
    ...
    async def test_done_list_is_newest_first_and_pages_by_cursor(
        ...
        for task in reversed(tasks):
            await storage.write_task(org, task)
        first = await storage.read_done_tasks(org, None, None, limit=2)
        assert [t.title for t in first] == ["d0", "d1"]
        last = first[-1]
        second = await storage.read_done_tasks(org, None, (last.updated_at, last.id), limit=2)
        assert [t.title for t in second] == ["d2", "d3"]
        last = second[-1]
        third = await storage.read_done_tasks(org, None, (last.updated_at, last.id), limit=2)
    ...
    async def test_mine_is_assigned_to_me_or_unassigned_and_created_by_me(
        ...
        for task in (mine_created, mine_assigned, given_away, theirs, mine_done):
            await storage.write_task(org, task)
        assert [t.title for t in await storage.read_open_tasks(org, me, limit=10)] == [
            "created by me",
            "assigned to me",
        ]
        assert len(await storage.read_open_tasks(org, None, limit=10)) == 4
        assert [t.title for t in await storage.read_done_tasks(org, me, None, limit=10)] == [
            "mine, done"
        ]
        assert await storage.read_done_tasks(org, other, None, limit=10) == []
