...
async def test_append_sequences_per_tenant_and_reads_back_by_seq(
    ...
    first = await manager.append(ann.org_id, make_event())
    second = await manager.append(ann.org_id, make_event("tasks.task.updated"))
    elsewhere = await manager.append(bob.org_id, make_event())
...
async def test_get_events_clamps_the_limit_and_tolerates_a_negative_cursor(
    ...
    ctx = context()
    for _ in range(3):
        await manager.append(ctx.org_id, make_event())
...
async def test_the_stream_is_read_with_read_and_is_append_only(manager: EventsManagerImpl) -> None:
    ...
    await manager.append(viewer.org_id, make_event())
