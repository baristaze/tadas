...
|---------|---------------|----------------------------|---------|
| portal | http://localhost:55173 | `portal:8080` | at `/login/dev`, by address: `owner@example.test` (owner) or `bob@example.test` (member) of Acme, or `admin@admin.test` (owner of Fabrikam, admin of Acme) (after `make seed`), each with a personal org beside; at `/login`, anyone through WorkOS's staging environment once `TADAS_WORKOS_API_KEY` (the staging Tadas App application's API key) is set in the shell or `.env` |
