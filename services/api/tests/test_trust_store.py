...
ENTRY = PACKAGE / "src/tadas/services/api" / "entry.py"
...
def test_the_console_script_starts_at_the_entry_module() -> None:
    ...
    assert scripts["tadas-api"] == "tadas.services.api.entry:main"
...
def test_the_entry_installs_the_trust_store_before_it_imports_the_process() -> None:
    ...
    process = [
        n.lineno
        for n in ast.walk(main)
        if isinstance(n, ast.ImportFrom) and n.module == "tadas.services.api.main"
    ]
...
def test_importing_the_process_leaves_ssl_alone() -> None:
    result = _run(
        ...
        "import tadas.services.api.main\n"
