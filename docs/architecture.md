...
and `queue` have chains today, and `admin` has no table yet. The migrate
command opens one connection per role, and every statement on it waits
for a lock `TADAS_DATABASE_MIGRATION_LOCK_TIMEOUT_SECONDS` (5) at most,
so a migration behind a long transaction does not hold the table's reads
and writes in the lock queue behind it for longer; a run that gives up
