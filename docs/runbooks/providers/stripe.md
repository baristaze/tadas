...
|-------|-----------------------------------|------------|-----------|
| Core | Customers | Write | create a customer per org; read it back when a delivery names it; delete it when its person deletes their account, or its owner deletes the team org |
...
| Billing | Subscriptions | Write | read a subscription; cancel it at the period's end or take that back; cancel it at once when its person deletes their account, or its owner deletes the team org; change the seat count |
...
| A deleted team org's subscription canceled at once and its customer deleted | The worker (`DELETE_ORG`) | When an owner deletes their team org; Stripe keeps the invoices |
