...
| A checkout or the portal waits about 30 seconds, then answers `503` | Stripe does not answer. Each call waits `TADAS_STRIPE_TIMEOUT_SECONDS` (10 seconds), and the SDK tries it twice more | Stripe's status page, then `/tadas/<env>/api`: `stripe <call> could not reach the backend: APIConnectionError` |
