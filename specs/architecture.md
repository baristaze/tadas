...
| [0025](../docs/adr/0025-rules-of-0-29-0-that-wait-for-their-feature.md) | DEL-31, DEL-38, OPS-15; Scale-Out as a Lever | The rules of 0.29.0 that wait for the feature they bind: a replicated overwrite caught by the digest, the app token, queue alarms, the enqueue permission, and `desired_count` kept out of `ignore_changes` on purpose. |
...
| [0027](../docs/adr/0027-a-task-slack-creates-takes-an-id-derived-from-the-delivery.md) | Identifiers; Idempotency on the Consumer Side | A task `/tadas add` creates takes a v7 id derived from the Slack delivery's key and the time it arrived, so the queue handing the same delivery back meets the task already there. Every other id is `new_id()`. |
