...
| [0027](../docs/adr/0027-plans-are-levers-and-the-processor-is-mirrored.md) | Queues (rate limits), Secrets | The payment processor's webhook route has no rate limit, with no path token to key one on; its key and signing secret are process credentials injected at start, not read through `SecretsInterface`. The record also names the product's choices: plans per org, a mirror written from reads, a 402 that releases the idempotency marker, keys kept and refused after a downgrade, and seeding outside the seat bound. |
